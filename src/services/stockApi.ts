...
export async function fetchStockData(
  symbol: string
): Promise<{ price: number; media200: number } | null> {
  try {
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        filter: [
          { left: "name", operation: "match", right: symbol.toUpperCase() },
        ],
        markets: ["brazil"],
        options: { lang: "pt" },
        range: [0, 1000],
    ...
    const data: { totalCount: number; data: Array<{ s: string; d: any[] }> } =
      await response.json();

    const stockData = data.data.find(
      (item) => item.s === `BMFBOVESPA:${symbol.toUpperCase()}`
    );
